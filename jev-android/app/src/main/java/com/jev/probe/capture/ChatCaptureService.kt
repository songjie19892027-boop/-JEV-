...
/**
 * The live capture service, registered in the Manifest under its own name. It
 * reads whichever adapted chat app is in the foreground, detects a new incoming
 * message from the other person, runs Jev analysis off the main thread, and
 * drives the floating overlay.
 *
...
class ChatCaptureService : AccessibilityService() {
     ...
     * 取出当前真正可用的本地服务地址（自愈）。
     *
     * Mac 的局域网地址由路由器分配，换网络就会变（实测一天内变过三次）。以前
     * 用户每次换网都得手动改设置页，所以这里做自动修复：配置的地址打不通
     * 就在本网段重新扫一次，找到后写回设置并返回新地址。
     *
     * 代价：每次分析多一次 /health 探测（局域网内通常几毫秒，失败才走扫描）。
     */
    private fun resolveLocalBase(): String {
        val configured = prefs.localJevUrl
        if (configured.isBlank()) return ""              // 未配置 → 走云端 Jev
        if (LocalDiscovery.probe(configured, 1500)) return configured

        val found = runCatching { LocalDiscovery.scan(8788) }.getOrNull() ?: return configured
        if (found != configured) {
            Log.i(TAG, "local service moved: $configured -> $found")
            prefs.localJevUrl = found
            main.post { overlay?.toast("Mac 地址已变，自动切到 $found") }
        }
        return found
    }
    ...
    private fun runAnalysis() {
        ...
        if (!prefs.canAnalyze()) {
            main.post {
                overlay?.showError("未配置判断后端：去设置里填 DeepSeek 密钥。" +
                    "判断和候选回复都靠这一个密钥。")
            }
            return
        }
