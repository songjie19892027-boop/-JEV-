...
class MainActivity : AppCompatActivity() {
    ...
    private fun build() {
        ...
        val backend = prefs.canAnalyze()   // 本地 NanoJev 服务 或 OpenRouter 密钥
        val ready = a11y && overlay && backend

        // Readiness card
        container.addView(statusCard(ready, a11y, overlay, backend))
        ...
        container.addView(actionRow("设置", "本地服务 · 密钥 · 模型 · 关系 · 透明度") {
    ...
    private fun statusCard(ready: Boolean, a11y: Boolean, overlay: Boolean, backend: Boolean): View {
        val c = cardBox()
        ...
        c.addView(checkLine(if (prefs.usesDeepSeek) "DeepSeek 密钥" else "判断密钥", backend, okWord = "已设", noWord = "未设"))
        return c
    }
