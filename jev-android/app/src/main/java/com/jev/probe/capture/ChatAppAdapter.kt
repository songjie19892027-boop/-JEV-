 ...
 *
 * The accessibility service reads the node tree of the foreground app. Some
 * chat apps expose plain resource-ids (Feishu/Lark); others need each adapter to
 * identify nodes structurally (by class name, depth and bounds) because their
 * resource-ids are not stable across versions.
 */
...
/** Feishu / Lark (com.ss.android.lark). Plain-text message bodies render as
 *  bare TextViews inside the bubble, so we collect the
